...
local function export_json_file(obj, as_key)
	...
	elseif kind == 'table' then
		...
		for k, v in pairs(obj) do
			...
			s[#s + 1] = export_json_file(k, true)
			s[#s + 1] = ':'
			s[#s + 1] = export_json_file(v)
		end
...
local function import_json_file(str, pos, end_delim)
	pos = pos or 1
	...
	if first == '{' then  -- Parse an object.
		...
		while true do
			key, pos = import_json_file(str, pos, '}')

			if key == nil then return obj, pos end
			if not delim_found then error('Comma missing between object items.') end

			pos = skip_delim(str, pos, ':', true)  -- true -> error if missing.
			obj[key], pos = import_json_file(str, pos)
	...
	elseif first == '[' then  -- Parse an array.
		...
		while true do
			val, pos = import_json_file(str, pos, ']')
