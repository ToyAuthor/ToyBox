...
std::string AnsiNumToStr(int key)
{
	if ( key<0 || key>127 )
	{
		toy::Ooops(TOY_MARK);
	...
	else
	{
		switch (key)
		{
			...
			case  10: return "[LF]" ;    // new line feed(Unix new line)
			case  11: return "[VT]" ;    // vertical tab
			case  12: return "[FF]" ;    // new page
			case  13: return "[CR]" ;    // carriage return(Classic Mac new line)
	...
	return "[Extended ASCLL Code or UTF-8]";
}
